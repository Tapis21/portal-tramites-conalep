{{-- resources/views/filament/modals/view-pdf-modal-practica.blade.php --}}
@php
    $practica = \App\Models\Practica::where('user_id', $record->user_id)->first();
    $pdfUrl = asset('storage/' . $record->archivo_pdf);
    
    $empresaNombre = $practica && $practica->empresa
        ? $practica->empresa->nombre
        : 'No asignada';
    $areaAsignada = $practica?->area_asignada ?? 'No asignada';
    $fechaInicio = $practica?->fecha_inicio
        ? date('d/m/Y', strtotime($practica->fecha_inicio))
        : 'No definida';
    $fechaFinalizacion = $practica?->fecha_limite_final
        ? date('d/m/Y', strtotime($practica->fecha_limite_final))
        : 'No definida';
    $horario = $practica?->horario
        ? $practica->horario->hora_inicio . ' - ' . $practica->horario->hora_fin
        : 'No definido';
    $contacto = $practica?->nombre_persona_carta ?? 'No definido';
    $horasRequeridas = $practica?->horas_requeridas ?? 'No definidas';
    $horasCompletadas = $practica?->horas_completadas ?? 'No definidas';
    ...
    <div style="background: #eff6ff; border-radius: 12px; border: 1px solid #bfdbfe; overflow: hidden; margin-bottom: 16px; box-shadow: 0 1px 2px rgba(0,0,0,0.05);">
        <div style="padding: 12px 20px; background: #dbeafe; border-bottom: 1px solid #bfdbfe;">
            <div style="display: flex; align-items: center; gap: 12px;">
                ...
                <span style="font-size: 14px; font-weight: 700; color: #1e40af; text-transform: uppercase; letter-spacing: 0.05em;">Datos de Prácticas Profesionales</span>
            </div>
        </div>
        <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 20px; padding: 20px;">
            ...
            <div>
                <span style="font-size: 11px; font-weight: 600; color: #2563eb; text-transform: uppercase; letter-spacing: 0.05em;">Horas requeridas</span>
                <p style="font-size: 14px; font-weight: 500; color: #111827; margin: 4px 0 0 0;">{{ $horasRequeridas }}</p>
            </div>
            <div>
                <span style="font-size: 11px; font-weight: 600; color: #2563eb; text-transform: uppercase; letter-spacing: 0.05em;">Horas completadas</span>
                <p style="font-size: 14px; font-weight: 500; color: #111827; margin: 4px 0 0 0;">{{ $horasCompletadas }}</p>
            </div>
        </div>
    </div>
